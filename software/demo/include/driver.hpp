...
public:
    Driver(std::shared_ptr<Position> shared_position, int wait_time, int latch_time,
           std::array<float, number_of_driver_pins> x_pins,
           std::array<float, number_of_driver_pins> y_pins,
           std::array<float, 2> latch_pins) : wait_time{std::chrono::nanoseconds(wait_time)},
    ...
    Driver(std::shared_ptr<Position> shared_position,
           std::array<float, number_of_driver_pins> x_pins,
           std::array<float, number_of_driver_pins> y_pins,
           std::array<float, 2> latch_pins) : shared_position{shared_position},
    ...
    std::thread spawn()
    {
        return std::thread(&Driver::run, this);
    }
